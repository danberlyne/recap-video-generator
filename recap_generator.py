...
ws = wb.active
id_cells = [cell for cell in ws['A'] if cell.value != 'FILENAME']
ids = [cell.value for cell in ws['A'] if cell.value != 'FILENAME']

# Pick out the specified clips from the files and normalise their audio.
video_clips = [VideoFileClip(str(Path('Videos', f'{id}'))).subclip(ws[f'B{id_cells[ids.index(id)].row}'].value, ws[f'C{id_cells[ids.index(id)].row}'].value).fx(afx.audio_normalize) for id in ids]
...
subs = [((int(sum(previous_clip.duration - custom_padding for previous_clip in video_clips[:video_clips.index(clip)])) + custom_padding, # Start time of subtitle
        ...
        ws[f'D{id_cells[video_clips.index(clip)].row}'].value + '\n' + ws[f'E{id_cells[video_clips.index(clip)].row}'].value + '\n' + ws[f'F{id_cells[video_clips.index(clip)].row}'].value) # Text of subtitle
        for clip in video_clips]
